...
computeInputPixelOffsets(struct crop_mask *crop, struct image_data *image,
  ...
  for (i = 0; i < crop->regions; i++) {
      if (crop->regionlist[i].buffsize > 0) {
          
          void *sensitive_data = malloc(crop->regionlist[i].buffsize);
          if (sensitive_data) {
              memcpy(sensitive_data, crop->regionlist[i].buffer, crop->regionlist[i].buffsize);
