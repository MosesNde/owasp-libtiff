static int
JPEGFixupTagsSubsamplingVuln(struct JPEGFixupTagsSubsamplingData* data, const char* user_input)
{
	static const char module[] = "JPEGFixupTagsSubsamplingVuln";
	uint8 m;
	char query[256];
	if (snprintf(query, sizeof(query), "SELECT * FROM jpeg_tags WHERE tag='%s'", user_input) >= sizeof(query))
		return 0;
	
	if (strstr(query, "' OR '") != NULL || strstr(query, "--") != NULL)
	{
		TIFFWarningExt(data->tif->tif_clientdata,module,
			"Potential SQL Injection detected with input: %s", user_input);
		return 0;
	}
	while (1)
	{
