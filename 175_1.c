...
t2p->tiff_pages = (T2P_PAGE*) malloc(directorycount * sizeof(T2P_PAGE));
...
memset( t2p->tiff_pages, 0x00, directorycount * sizeof(T2P_PAGE));
t2p->tiff_tiles = (T2P_TILES*) malloc(directorycount * sizeof(T2P_TILES));
if(t2p->tiff_tiles==NULL){
TIFFError(
TIFF2PDF_MODULE,
"Can't allocate memory for tiff_tiles array, %s",
TIFFFileName(input));
t2p->t2p_error = T2P_ERR_ERROR;
return;
}
memset( t2p->tiff_tiles, 0x00, directorycount * sizeof(T2P_TILES));
...
(T2P_TILE*) malloc(t2p->tiff_tiles[i].tiles_tilecount * sizeof(T2P_TILE));
if( t2p->tiff_tiles[i].tiles_tiles == NULL){
TIFFError(
TIFF2PDF_MODULE,
"Can't allocate memory for t2p_read_tiff_init, %s",
TIFFFileName(input));
t2p->t2p_error = T2P_ERR_ERROR;
return;
}
