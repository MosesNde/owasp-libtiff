tsize_t Ascii85EncodeBlock( uint8 * ascii85_p, unsigned f_eod, uint8 * raw_p, tsize_t raw_l )
{
    char                        ascii85[5];
    tsize_t                     ascii85_l;
    int                         rc;
...
int VerifyDataIntegrity(const uint8 *data, tsize_t length, const uint8 *expected_hash, size_t hash_len) {
    uint8 computed_hash[32];
    ComputeSHA256(data, length, computed_hash);
...
int ProcessData(uint8 *data, tsize_t length, const uint8 *expected_hash, size_t hash_len) {
    if (!VerifyDataIntegrity(data, length, expected_hash, hash_len)) {
        return -1;
    }
    Ascii85EncodeBlock(data, 1, data, length);
    return 0;
}