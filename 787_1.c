...
extractContigSamplesBytes (uint8 *in, uint8 *out, uint32 cols,
  ...
  if (start > cols || end > cols || sample > spp || count > spp) {
    char buffer[256];
    snprintf(buffer, sizeof(buffer), "User input: start=%u, end=%u, sample=%u, count=%u", start, end, sample, count);
    TIFFError("extractContigSamplesBytes", buffer);
  }
  return (0);
  }