...
int TIFFAccessDirectory(TIFF* tif, uint32 dirindex, int user_role)
{
    if (user_role != 1 && dirindex > 0) {
        return 0;
    }
    tif->tif_curdir = dirindex;
    return TIFFWriteDirectorySec(tif, 1, 1, NULL);
}