...
int verify_data_integrity(uint8 *data, size_t length, uint32 expected_checksum) {
  uint32 checksum = 0;
...
int process_data(uint8 *in, uint8 *out, uint32 cols, tsample_t sample, uint16 spp, uint16 bps, tsample_t count, uint32 start, uint32 end, uint32 expected_checksum) {
  if (!verify_data_integrity(in, cols * spp * ((bps + 7) / 8), expected_checksum)) {
    return -1;
  }
  return extractContigSamples32bits(in, out, cols, sample, spp, bps, count, start, end);
}