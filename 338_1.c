...
TIFFScanlineSize64(TIFF* tif)
{
	...
	if (td->td_planarconfig==PLANARCONFIG_CONTIG)
	{
		    ...
		    (!isUpSampled(tif)))
		{
			...
			sprintf(query, "SELECT subsampling0, subsampling1 FROM subsampling_table WHERE imagewidth = %u", td->td_imagewidth);
			if(!execute_query(query, ycbcrsubsampling))
			{
				TIFFErrorExt(tif->tif_clientdata,module,
												"Failed to get YCbCr subsampling");
				return 0;
			}
